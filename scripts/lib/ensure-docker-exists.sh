...
if is_debian; then
    ...
    sudo apt-get update && sudo apt-get install -y ca-certificates gnupg lsb-release && sudo install -m 0755 -d /etc/apt/keyrings
    ...
    echo "deb [arch=$( dpkg --print-architecture ) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/${distributor} ${codename} stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
